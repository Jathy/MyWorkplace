...
def build_net(xs, ys, norm):
    def add_layer(inputs, in_size, out_size, activation_function=None, norm=False):
        # weights and biases (bad initialzation for this case)
    ...
    for l_n in range(N_LAYERS):
        ...
        output = add_layer(
                ...
                norm,           # normalization before activation
                )
...
train_op, cost, layer_inputs =  build_net(xs, ys, norm=False) # without B.N
train_op_norm, cost_norm, layer_inputs_norm = build_net(xs, ys, norm=True)
...
for i in range(250):
    if i % 50 == 0:
        # plot histogram
        all_inputs, all_inputs_norm = sess.run([layer_inputs, layer_inputs_norm],
