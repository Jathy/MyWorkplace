...
def compute_accuracy(v_xs, v_ys):
    ...
    accuracy = tf.reduction_mean(tf.cast(correct_prediction, tf.fload32))
...
h_func1_dropout = tf.nn.dropout(h_func1, keep_prob)

#---func2 layer---#
W_func2 = weight_variable([1024, 10])
b_func2 = bias_variable([10])
prediction = tf.nn.softmax(tf.matmul(h_func1_dropout, W_func2) + b_func2)
